    ...
    Service.prototype.deploy = function(branch, cb) {
      ...
      return this.create(function(err) {
        ...
        return local("git", ["push", _this.repoUrl, branch, "-f"], function(err) {
          ...
          console.log("[√] pushed");
          installCommand = this.makeInstallCommand() + "\n" + this.makeRestartCommand();
          return ssh(server, installCommand, function(err) {
            var command, kill;
            if (err != null) {
              return cb(err);
            }
            console.log();
            command = this.log(1, cb);
            kill = function() {
              return command.kill();
            };
